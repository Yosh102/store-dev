...
import { Lock, BellIcon, CalendarIcon, ChevronRight } from "lucide-react"
...
export default function GroupNews() {
  ...
  const updateUrl = (page: number, sort: 'asc' | 'desc') => {
    ...
    router.push(`/group/${slug}/news?${params.toString()}`)
  }
  ...
  useEffect(() => {
    const fetchGroupNews = async () => {
      ...
      try {
        ...
        const newsQuery = query(
          ...
          where("isOfficialAnnouncement", "==", true),
        ...
        // ページネーション用に情報を設定
        setTotalNews(allNews.length);
        setTotalPages(Math.ceil(allNews.length / NEWS_PER_PAGE));
        
        // 現在のページのデータだけを抽出
        const startIndex = (currentPage - 1) * NEWS_PER_PAGE;
        const paginatedNews = allNews.slice(startIndex, startIndex + NEWS_PER_PAGE);
    ...
    fetchGroupNews();
  }, [slug, user, currentPage, sortOrder]);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-bold">{groupInfo.name}からのお知らせ</h1>
          </div>
          <p className="text-gray-600">
            {groupInfo.name}からの公式なお知らせが表示されています
          </p>
        </div>
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="text-sm text-gray-500">
            全 {totalNews} 件中 {totalNews > 0 ? (currentPage - 1) * NEWS_PER_PAGE + 1 : 0} - {Math.min(currentPage * NEWS_PER_PAGE, totalNews)} 件を表示
          </div>
          <div className="flex items-center gap-2">
        ...
        {news.length === 0 ? (
          <Alert>
            <AlertDescription>お知らせが見つかりませんでした。</AlertDescription>
          </Alert>
        ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {news.map((item) => (
                <Link href={`/post/${item.id}`} key={item.id} className="block group">
                  <div className="rounded-lg overflow-hidden shadow hover:shadow-md transition-shadow">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between items-center">
                        <time className="text-sm text-gray-500" dateTime={item.publishDate.toDate().toISOString()}>
                          {format(item.publishDate.toDate(), "yyyy.MM.dd")}
                        </time>
                        {item.membersOnly && (
